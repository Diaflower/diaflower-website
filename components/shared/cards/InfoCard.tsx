...
export default function InfoCard({title, desc, link, img, alt, href, location}: Props) {
    ...
    return (
        <div className="w-full relative">
            ...
            <div className={`${isHomePage?"absolute top-2 left-2 md:static md:p-6 flex flex-col md:items-center md:text-center":"p-6 flex flex-col items-center text-center"}`}>
                <h2 className="text-2xl font-bold mb-2">{title}</h2>
                    <p className= {`${isHomePage?"hidden md:flex mb-4":"mb-4 "}`}>
                        {desc}
                    </p>
                    <Link
                        href={href}
